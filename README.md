...
| `key_collisions` | List items dropped because a later sibling of the same list claimed their database key — the last one is written (`path` → `kept_path`). `kind` is `duplicate` (same values, nothing lost) or `conflicting` (`differing_fields` were lost — either a noisy repeat, or two real things behind one key value: mark the discriminating property identifying and republish). Also stored on the record. |
